...
other_data = spark.read.csv("other_data.csv", header=True, inferSchema=True)
joined_data = preprocessed_data.join(other_data, "col1", "left")

filtered_data.write.mode("overwrite").parquet("filtered_data.parquet")
grouped_data.write.mode("overwrite").parquet("grouped_data.parquet")
new_data.write.mode("overwrite").parquet("new_data.parquet")
joined_data.write.mode("overwrite").parquet("joined_data.parquet")
